# This agent uses a RNN and is fed the past states and past actions to choose the best jamming power
# The agent learns that it only has to jam once every 4 actions to keep the modulation from increasing
# The result is that it earns the best possible average reward of 3.5
...
# each state is a list of length 2, composed of the current modulation observed and the last action taken
num_inputs = 2
num_hidden = 16
num_timesteps = 5
...
# builds RNN for predicting q-values based on last num_timesteps of state (including current mod and last action)
...
def train():
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            # create pair of state and last_action
            state_action_pair = np.append(obs, last_action)
